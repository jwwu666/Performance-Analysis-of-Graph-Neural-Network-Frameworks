...
#         print(batch_graphs.edata['pseudo'])
...
def train_dgl(model_name, model, optimizer, device, data_loader):
    ...
    for iter, (batch_graphs, batch_labels) in enumerate(data_loader):
        ...
        nb_data += batch_labels.size(0)
        torch.cuda.synchronize()
        print("ACC:", time.perf_counter() - tt)
...
train_loader, valid_loader, test_loader = GraphDataLoader('dgl',
        dataset, batch_size=64, device=torch.device('cuda'),
        seed=0, shuffle=True,
...
model = MoNet_DGL(num_features, 80, n_classes, 2, 3, device).to(device)
...
for epoch in range(1, 201):
    torch.cuda.synchronize()
    t1 = time.time()
    train_loss, train_acc, optimizer= train_dgl('monet', model,  optimizer, device, train_loader)
    torch.cuda.synchronize()
    ...
    test_loss, test_acc = test_dgl('monet', model, device, test_loader)
