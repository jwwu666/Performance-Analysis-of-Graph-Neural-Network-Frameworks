...
model = MoNet_PYG(num_features, 80, n_classes, 2, 3).to(device)
...
for epoch in range(1, 201):
    t1 = time.time()
    train_loss, train_acc, optimizer= train_pyg('monet', model, train_loader, device, optimizer)
    ...
    test_loss, test_acc = test_pyg('monet', model, test_loader, device)
