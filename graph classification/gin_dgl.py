...
dataset = LegacyTUDataset(name='DD')
...
model = GIN_DGL(num_features, 80, n_classes).to(device)
...
for epoch in range(1, 201):
    torch.cuda.synchronize()
    t1 = time.time()
    train_loss, train_acc, optimizer = train_dgl('gin', model, optimizer, device, train_loader)
    torch.cuda.synchronize()
    ...
    test_loss, test_acc = test_dgl('gin', model, device, test_loader)
