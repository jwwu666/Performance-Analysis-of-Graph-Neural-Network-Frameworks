...
def train_epoch(model_name, model, optimizer, device, data_loader):#model, optimizer, device, data_loader, epoch):
    model.train()
    epoch_loss = 0
    epoch_train_acc = 0
    nb_data = 0
...
def evaluate_network(model_name, model, device, data_loader):
    model.eval()
    epoch_test_loss = 0
    epoch_test_acc = 0
    nb_data = 0
...
for graph in dataset.graph_lists:
            ...
            if 'feat' not in graph.edata.keys():
                ...
                graph.edata['feat'] = torch.zeros(graph.number_of_edges(), edge_feat_dim)
...
model = Gated_DGL(num_features, 96, n_classes, 0.5, device).to(device)
...
for epoch in range(1, 201):
    t1 = time.time()
    train_loss, train_acc, optimizer= train_dgl('gate', model,  optimizer, device, train_loader)
    ...
    test_loss, test_acc = evaluate_network('gate', model, device, test_loader)
    scheduler.step(test_loss)
    print('Epoch: {:03d}, Train Loss: {:.4f}, '
          'Train Acc: {:.4f}, Test Acc: {:.4f}, Time: {:.4f}'.format(epoch, train_loss,
                                                                     train_acc, test_acc, np.mean(dur)))
    # print("memory_allocated: ", torch.cuda.max_memory_allocated(), "memory_cached : {}",
