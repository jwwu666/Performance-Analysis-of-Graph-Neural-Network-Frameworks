...
print(dataset.num_node_attributes, dataset.num_node_features, dataset.num_node_labels)

t2 = time.time()
train_loader, valid_loader, test_loader = GraphDataLoader('pyg', dataset, batch_size=64, device=torch.device('cuda'), seed=0, shuffle=True, split_name='fold10').train_valid_test_loader()
...
model = SAGE_PYG(num_features, 96, n_classes, 0.5).to(device)
...
for epoch in range(1, 201):
    t1 = time.time()
    train_loss, train_acc, optimizer= train_pyg('sage', model, train_loader, device, optimizer)
    ...
    test_loss, test_acc = test_pyg('sage', model, test_loader, device)
