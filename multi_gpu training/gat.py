...
class GAT_DGL(nn.Module):
    def __init__(self, in_feats, n_classes, n_hidden, n_heads, drop):
        ...
        self.layers = nn.ModuleList()
        self.embedding_h = nn.Linear(in_feats, n_hidden * n_heads)
        self.layers.append(GAT_DGLConv(in_feats, n_hidden, n_heads, allow_zero_in_degree = True))
        for _ in range(2):
            self.layers.append(GAT_DGLConv(n_hidden * n_heads, n_hidden, n_heads, allow_zero_in_degree = True))

        self.layers.append(GAT_DGLConv(n_hidden * n_heads, n_hidden, 1, allow_zero_in_degree = True))
