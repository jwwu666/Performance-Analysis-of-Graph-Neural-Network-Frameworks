...
class DataParallel(torch.nn.DataParallel):
    ...
    def forward(self, data_list):
        ...
        if not self.device_ids or len(self.device_ids) == 1:  # Fallback
            data = dgl.batch(data_list).to(self.src_device)
        ...
        print("batch time:", time.time() - t)
        t1 = time.time()
        replicas = self.replicate(self.module, self.device_ids[:len(inputs)])
        torch.cuda.synchronize()
        print("model to gpu:", time.time() - t1)
        t2 = time.time()
        outputs = self.parallel_apply(replicas, inputs, None)
        torch.cuda.synchronize()
        print("model compute:", time.time() - t2)
        t3 = time.time()
        re = self.gather(outputs, self.output_device)
        torch.cuda.synchronize()
        print("data trans:", time.time() - t3)
        del inputs
        del replicas
        del outputs
        return re
    ...
    def scatter(self, data_list, device_ids):
        ...
        #print(split)
        t = time.time()
        input = [dgl.batch(data_list[split[i]:split[i + 1]]).to(
