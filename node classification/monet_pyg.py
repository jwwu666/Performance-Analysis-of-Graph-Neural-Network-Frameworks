...
model, data = MoNet_PYG(in_feats, 16, n_classes, 2, 3).to(device), data.to(device)
...
def train():
    model.train()
    optimizer.zero_grad()
    F.nll_loss(model(x, edge_index, edge_attr)[data.train_mask], data.y[data.train_mask]).backward()
    optimizer.step()
'''
...
for epoch in range(1, 201):
    ...
    model.train()
    loss = F.nll_loss(model(x, edge_index, edge_attr)[data.train_mask], data.y[data.train_mask])
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
