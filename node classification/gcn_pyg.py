...
model, data = GCN_PYG(in_feats, n_classes).to(device), data.to(device)
...
def train():
    model.train()
    optimizer.zero_grad()
    F.nll_loss(model(x, edge_index, edge_weight)[data.train_mask], data.y[data.train_mask]).backward()
    optimizer.step()
...
def test():
    model.eval()
    logits, accs = model(x, edge_index, edge_weight), []
...
for epoch in range(1, 201):
    ...
    model.train()
    loss = F.nll_loss(model(x, edge_index, edge_weight)[data.train_mask], data.y[data.train_mask])
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
